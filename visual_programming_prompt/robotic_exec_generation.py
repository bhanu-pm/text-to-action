...
def exec_steps(instruction_task, task_id=None, folder="generated_tasks"):
        ...
        while trials < 5:
            try:
                ...
                response = openai.ChatCompletion.create(
                    model="gpt-3.5-turbo",
                    messages=[
                        {"role": "system", "content": "You are a helpful assistant that generates Python code for robotic manipulation tasks."},
                ...
                if len(result.split('\n')) > 3:
                    ...
                    print("The result is too short, retrying...")
            except Exception as e:
                print(f"Error in API call: {e}")
            
            trials += 1
        
        raise Exception("Failed to generate valid code after multiple attempts")

# You would need to define prompt_base before using the function
# Example:
# prompt_base = """Your base prompt template with INSERT TASK HERE placeholder"""
...
def exec_steps_off(
    ...
    if task == "rotate":
        codes =  rotate_exec_steps_off(query, degrees)
    elif task == "visual_manipulation":
        codes =  visual_manipulation_steps_off(query_2, query_1)
    elif task == "pick_in_order_then_restore":
        codes = pick_in_order_then_restore_off(query_2, query_3,  query_1)
    elif task == "rearrange":
        codes = rearrange_off()
    elif task == "rearrange_then_restore":
        codes = rearrange_then_restore_off()
    elif task == "scene_understanding":
        codes = scene_understanding_off(query_1, query_2)
...
def rotate_exec_steps_off(query, degrees):
...
def visual_manipulation_steps_off(query_1, query_2):
    codes = [
        "MASKS=SAM(image=IMAGE)",
        "OBJS, MASKS=ImageCrop(image=IMAGE, masks=MASKS)",
...
def pick_in_order_then_restore_off(query_1, query_2, query_3):
...
def rearrange_then_restore_off():
    codes = [
        "MASKS_OBS=SAM(image=IMAGE)", 
        "OBJS_OBS, MASKS_OBS=ImageCrop(image=IMAGE, masks=MASKS_OBS)", 
        "MASKS_GOAL=SAM(image=templates['scene'])",
...
def scene_understanding_off(query_1, query_2):
